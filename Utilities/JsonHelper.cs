...
{
    private static readonly JsonSerializerSettings _settings = new() { TypeNameHandling = TypeNameHandling.Auto, Formatting = Formatting.Indented };
    ...
    public static string Serialize<T>(T instance, JsonSerializerSettings? settings = null)
        => JsonConvert.SerializeObject(instance, Formatting.Indented, settings ?? _settings);
    ...
    public static T Deserialize<T>(string json, JsonSerializerSettings? settings = null)
        => (T)JsonConvert.DeserializeObject(json, typeof(T), settings ?? _settings)!;
